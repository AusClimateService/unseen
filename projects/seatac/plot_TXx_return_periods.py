...
def _main(args):
    ...
    ds_ensemble_stacked = ds_ensemble.stack(
    ...
    full_model_return_period = return_period(
        ds_ensemble_stacked["tasmax"].values, threshold
    )
    logging.info(
        f"TXx={threshold}C return period in full model ensemble: {full_model_return_period}"
    )

    gev_shape, gev_loc, gev_scale = indices.fit_gev(
        ds_ensemble_stacked["tasmax"].values, use_estimates=True
    )
    ...
    logging.info(
        f"TXx={threshold}C return period from GEV fit to full model ensemble: {full_gev_return_period}"
    )

    df_model_return_period = pd.DataFrame(
        [full_model_return_period] * n_repeats, columns=[population_size]
    )
    df_gev_return_period = pd.DataFrame(
        [full_gev_return_period] * n_repeats, columns=[population_size]
    )
    ...
    for sample_size in [10, 50, 100, 500, 1000, 5000, 10000]:
        ...
        for resample in range(n_repeats):
            ...
            # random_indexes.sort()
            model_subsample = ds_ensemble_stacked["tasmax"].isel(
                {"sample": random_indexes}
            )
            model_return_period = return_period(model_subsample.values, threshold)
            model_estimates.append(model_return_period)
            gev_shape, gev_loc, gev_scale = indices.fit_gev(
                model_subsample.values, use_estimates=False
            )
            gev_data = gev.rvs(
                gev_shape, loc=gev_loc, scale=gev_scale, size=args.gev_samples
            )
    ...
    df_model_return_period = df_model_return_period.reindex(
        sorted(df_model_return_period.columns), axis=1
    )
    df_gev_return_period = df_gev_return_period.reindex(
        sorted(df_gev_return_period.columns), axis=1
    )
