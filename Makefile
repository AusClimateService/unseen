...
${OBS_PROCESSED_FILE} : ${OBS_DATA} ${OBS_METADATA}
	${PYTHON} ${SCRIPT_DIR}/preprocess.py $< obs $@ --metadata_file $(word 2,$^) ${OBS_IO_OPTIONS}
...
${FCST_ENSEMBLE_FILE} : ${FCST_METADATA}
	${PYTHON} ${SCRIPT_DIR}/preprocess.py ${FCST_DATA} forecast $@ --metadata_file $< ${FCST_IO_OPTIONS} --reset_times --output_chunks lead_time=50 --dask_config ${DASK_CONFIG}
