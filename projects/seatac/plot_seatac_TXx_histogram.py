...
def _main(args):
    ...
    logging.info(
        f"Observations GEV fit: shape={obs_shape}, location={obs_loc}, scale={obs_scale}"
    )

    ds_raw = fileio.open_file(args.raw_model_file)
    ds_raw_stacked = ds_raw.stack(
        {"sample": ["ensemble", "init_date", "lead_time"]}
    ).compute()
    raw_shape, raw_loc, raw_scale = indices.fit_gev(
        ds_raw_stacked["tasmax"].values, use_estimates=True
    )
    logging.info(
        f"Model (raw) GEV fit: shape={raw_shape}, location={raw_loc}, scale={raw_scale}"
    )

    ds_bias = fileio.open_file(args.bias_corrected_model_file)
    ds_bias_stacked = ds_bias.stack(
        {"sample": ["ensemble", "init_date", "lead_time"]}
    ).compute()
    bias_shape, bias_loc, bias_scale = indices.fit_gev(
        ds_bias_stacked["tasmax"].values, use_estimates=True
    )
    logging.info(
        f"Model (bias corrected) GEV fit: shape={bias_shape}, location={bias_loc}, scale={bias_scale}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "bias_corrected_model_file", type=str, help="Model file (bias corrected)"
    )
